...
from playwright.async_api import Playwright, async_playwright, Page, expect
import os
import asyncio
...
class DouYinVideo(object):
    ...
    async def set_none_thumbnail(self, page: Page):
        douyin_logger.info('  [-] 正在设置视频封面...')

        try:
        # 1. 点击外部的“选择封面”按钮
            await page.locator('div:has-text("选择封面")').last.click()

        # 2. 等待弹窗出现
            modal = page.locator(".dy-creator-content-modal-content")
            await modal.wait_for(state="visible", timeout=5000)

        # 3. 确保切换到“设置竖封面” tab
            upload_tab = modal.locator("div[role='tab']").filter(has_text="设置竖封面").first
            if await upload_tab.count() > 0:
                await upload_tab.click()
                await asyncio.sleep(0.5)

            douyin_logger.info("  [-] 正在点击上传按钮...")
        
        # 4. 智能等待“完成”按钮变亮（不再 disable）
            finish_btn = modal.locator('button').filter(has_text="完成")
            await expect(finish_btn).not_to_have_attribute("disabled", "", timeout=20000)

            await finish_btn.click()
            douyin_logger.success('  [+] 封面设置结束')

        # 5. 等待弹窗消失
            await page.wait_for_selector(".dy-creator-content-modal-content", state="hidden", timeout=5000)

        except Exception as e:
            douyin_logger.error(f"  [x] 封面上传流程出错: {e}")
            await page.screenshot(path="error_set_thumbnail.png")
    
    async def set_thumbnail(self, page: Page, thumbnail_path: str):
        if not thumbnail_path:
            self.set_none_thumbnail(page)
        ...
        # ==================== 核心修改 ====================
        # 方案：不再去猜哪个 hidden input 是对的。
        # 直接点击截图里那个可见的【+ 上传封面】按钮，拦截文件选择器。
        
            async with page.expect_file_chooser() as fc_info:
            # 定位截图右下角那个“+ 上传封面”按钮
            # 注意：截图里文字是 "上传封面"，不要写成 "点击上传"
                upload_btn = modal.locator("div, button").filter(has_text="上传封面").last
