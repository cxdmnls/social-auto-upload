...
def getValidAccounts():
    # 前端账号页会调用这个接口刷新列表。这里先返回数据库中的账号状态，
    # 避免每次刷新都启动浏览器逐个平台验证 Cookie。
    return getAccounts()

def normalize_platform_type(platform):
    """把前端/数据库里可能出现的平台写法统一成 1-5。"""
    platform_str = str(platform or '').strip().lower()
    platform_map = {
        '1': 1,
        'xhs': 1,
        'xiaohongshu': 1,
...
def platform_display_name(platform_type):
    return {
        1: '小红书',
        2: '视频号',
        3: '抖音',
        4: '快手',
        5: 'B站',
    }.get(platform_type, '未知平台')

def validate_account_cookie(account):
    """发布前统一校验 Cookie。返回 (是否有效, 平台编号, Cookie 文件名, 原因)。"""
    platform_type = normalize_platform_type(account.get('type'))
    cookie_identifier = account.get('filePath') or account.get('cookie_path')

    if not platform_type:
        return False, None, cookie_identifier, '未知平台'
    if not cookie_identifier:
        return False, platform_type, cookie_identifier, 'Cookie文件缺失'

    cookie_path = BASE_DIR / "cookiesFile" / cookie_identifier
    if not cookie_path.exists():
        return False, platform_type, cookie_identifier, 'Cookie文件不存在'

    try:
        is_valid = asyncio.run(check_cookie(platform_type, cookie_identifier))
    except Exception as e:
        logger.error(
            f"Cookie校验异常: account={account.get('userName')}, "
            f"platform={platform_type}, cookie={cookie_identifier}, error={e}",
            exc_info=True
        )
        return False, platform_type, cookie_identifier, f'Cookie校验异常: {e}'

    if not is_valid:
        return False, platform_type, cookie_identifier, '登录已过期'

    return True, platform_type, cookie_identifier, ''
...
def run_async_login(type_, id_, status_queue):
    ...
    try:
        ...
        if t_str in ['mock', 'test', 'qrcode-test']:
            svg = """<svg xmlns="http://www.w3.org/2000/svg" width="220" height="220" viewBox="0 0 220 220"><rect width="220" height="220" fill="#fff"/><rect x="18" y="18" width="54" height="54" fill="#111"/><rect x="30" y="30" width="30" height="30" fill="#fff"/><rect x="148" y="18" width="54" height="54" fill="#111"/><rect x="160" y="30" width="30" height="30" fill="#fff"/><rect x="18" y="148" width="54" height="54" fill="#111"/><rect x="30" y="160" width="30" height="30" fill="#fff"/><rect x="94" y="92" width="18" height="18" fill="#111"/><rect x="124" y="92" width="18" height="18" fill="#111"/><rect x="92" y="124" width="54" height="18" fill="#111"/><rect x="160" y="112" width="18" height="18" fill="#111"/><rect x="112" y="160" width="18" height="18" fill="#111"/><text x="110" y="210" font-size="14" text-anchor="middle" fill="#333">SAU QR TEST</text></svg>"""
            image_data = "data:image/svg+xml;base64," + base64.b64encode(svg.encode("utf-8")).decode("ascii")
            ...
            return
        if t_str in ['xiaohongshu', 'xhs', '小红书', '1']:
            func = xiaohongshu_cookie_gen
        elif t_str in ['shipinhao', 'tencent', 'channels', '视频号', '2']:
            func = get_tencent_cookie
        elif t_str in ['douyin', '抖音', '3']:
            func = douyin_cookie_gen
        elif t_str in ['kuaishou', 'ks', '快手', '4']:
            func = get_ks_cookie
        elif t_str in ['bilibili', 'bili', '哔哩哔哩', 'b站', '5']:
...
def sse_stream(status_queue):
    ...
    while True:
        if not status_queue.empty():
            ...
            yield f"data: {msg}\n\n"
            # 简单判断结束条件
            if str(msg) in ["200", "500"] or any(k in str(msg) for k in ["流程结束", "发生异常", "错误:", "Error", "Failed"]):
                time.sleep(1) # 等待前端接收
                break
        else:
            time.sleep(0.1)
...
def postVideo():
    ...
    for account in real_account_list:
        ...
        is_cookie_valid, platform_type, cookie_identifier, invalid_reason = validate_account_cookie(account)
        display_platform = platform_display_name(platform_type)

        if not is_cookie_valid:
            logger.warning(
                f"发布前Cookie校验失败: 账号={acc_name}, 平台={display_platform}, "
                f"Cookie={cookie_identifier}, 原因={invalid_reason}"
            )
            if invalid_reason == '未知平台':
                results.append(f"未知平台({acc_name}): ❌ {account.get('type')}")
            else:
                results.append(f"{display_platform}({acc_name}): 登录已过期，请重新扫码")
            continue

        # 将字符串放入列表传给 myUtils
        acc_list = [cookie_identifier] 

        logger.info(f"正在调度任务: 账号={acc_name}, 标识={cookie_identifier}, 平台={display_platform}")
...
if __name__ == '__main__':
    ...
    print("Backend service started: http://127.0.0.1:5409")
    print("Open index.html and make sure it uses port 5409")
